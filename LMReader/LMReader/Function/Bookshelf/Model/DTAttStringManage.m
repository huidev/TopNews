...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.lBookmarks = [NSMutableArray array];
        self.lBookideals = [NSMutableArray array];
    }
    return self;
}
...
- (NSArray *)framesetterOfChapter:(NSInteger)index;
{
...
//        _indexOfChapter = index;;
    ...
    if (_lChapters.count>index) {
        ...
        NSString *html = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"book" ofType:@"html"] encoding:NSUTF8StringEncoding error:nil];
        NSString *fontColor;
        if ([LMGoble sharedGoble].theme==STThemeWhite) {
            fontColor = @"rgb(0,0,0)";
        }else if ([LMGoble sharedGoble].theme==STThemeBlack)
        {
            fontColor = @"#eeeeee";
        }
        NSData *data = [[NSString stringWithFormat:html,fontColor,[LMGoble sharedGoble].fontSize,[LMGoble sharedGoble].fontSize+8,content] dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)_parseChapter
{
    ...
    NSArray *infos = [doc nodesForXPath:@"//nb" error:nil];
    NSArray *descs = [doc nodesForXPath:@"//description" error:nil];
    GDataXMLNode *idNode = [doc firstNodeForXPath:@"//id" error:nil];
    self.bookId = [idNode stringValue];
    GDataXMLNode *authorNode = [doc firstNodeForXPath:@"//author" error:nil];
    self.author = [authorNode stringValue];
    GDataXMLNode *publisherNode = [doc firstNodeForXPath:@"//publisher" error:nil];
    self.publisher = [publisherNode stringValue];
    for(GDataXMLElement *element in descs)
    {
        self.brief = [element stringValue];
    }
    for(GDataXMLElement *element in infos)
    {
        NSArray *atts = [element attributes];
        for (GDataXMLNode *node in atts)
        {
            if ([[node name]isEqualToString:@"title"]) {
                self.bookName = [node stringValue];
            }
        }
    }
...
- (NSAttributedString *)_attributedStringForSnippetUsingiOS6Attributes:(BOOL)useiOS6Attributes
{
    ...
    [_lBookmarks removeAllObjects];
    [_lBookideals removeAllObjects];
    [_lBookmarks addObjectsFromArray:[[DatebaseManage sharedDatebase] fetchBookMark:self.bookId chapterIndex:-1]];
    [_lBookideals addObjectsFromArray:[[DatebaseManage sharedDatebase] fetchBookIdeal:self.bookId chapterIndex:-1]];
    // Load HTML data
